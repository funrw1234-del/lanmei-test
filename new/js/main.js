...
(function () {
  ...
  // первый вопрос («Будут ли доплаты после оплаты?») открыт сразу
  const firstQ = $('#acc .acc__item .acc__q');
  ...
  async function submitLead(data, emailTemplateId) {
    ...
    // превью /new/ и локальный просмотр — заявки не отправляем, чтобы не засорять рабочий канал команды
    if (/^(localhost|127\.|\[::1\])/.test(location.hostname) || location.pathname.indexOf('/new/') === 0) {
      window.alert('Это предпросмотр новой версии сайта — заявка не отправлена.');
      console.info('Локальный просмотр: заявка не отправлена', data);
      return { ok: false };
    }
  ...
  if (leadForm) {
    ...
    // Шаги с плитками после выбора сами переходят дальше — не нужно жать
    // «Дальше». Шаги с текстовым полем (ссылка, количество, город) — нет.
